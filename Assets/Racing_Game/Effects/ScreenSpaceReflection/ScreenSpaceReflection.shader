    ...
    SubShader
    {
        ...
        Pass
		{
			...
			half4 frag(Varyings input) : SV_Target
			{
				...
				if (rayHit.distance > REAL_EPS)
				{
					...
					return half4(lerp(sceneColor, reflectColor, saturate(reflectivity*_Intensity + fresnel) * EdgeOfScreenFade(UnityStereoTransformScreenSpaceTex(screenUV))), 1.0);
				}
				else
				{
		...
		Pass
		{
			...
			half4 frag(Varyings input) : SV_Target
			{
				...
				return half4(lerp(sceneColor, reflectColor, saturate(reflectivity*_Intensity + fresnel) * EdgeOfScreenFade(UnityStereoTransformScreenSpaceTex(screenUV)) * fadeSmoothness), 1.0);
			}
			ENDHLSL
		}
