...
[Serializable, VolumeComponentMenuForRenderPipeline("Lighting/Screen Space Reflection (URP)", typeof(UniversalRenderPipeline))]
    ...
    public class ScreenSpaceReflection : VolumeComponent, IPostProcessComponent
    {
        ...
        public SSRAlgorithmParameter algorithm = new(value: Algorithm.Approximation, overrideState: true);
        [InspectorName("Intensity"), Tooltip("Reflection intensity.")]
        public ClampedFloatParameter Intensity = new(value: 1.0f, min: 0.0f, max: 7f, overrideState: true);
        [InspectorName("Minimum Smoothness"), Tooltip("SSR ignores a pixel if its smoothness value is lower than this value.")]
        public ClampedFloatParameter minSmoothness = new(value: 0, min: 0.0f, max: 1.0f, overrideState: true);
        ...
        public ClampedFloatParameter fadeSmoothness = new(value: 1f, min: 0.0f, max: 1.0f, overrideState: true);
        [InspectorName("Screen Edge Fade Distance"), Tooltip("Fades out screen space reflection when it is near the screen boundaries.")]
        public ClampedFloatParameter edgeFade = new(value: 0, min: 0.0f, max: 1.0f, overrideState: true);
